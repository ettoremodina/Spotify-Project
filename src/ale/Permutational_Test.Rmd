...
rejecting H0: popularity=0.090, acousticness=0.023, energy=0.047, loudness=0.060, tempo=0.038
...
rejecting H0: popularity=0.097. Unbalanced data

```{r}
res_importanza.musica<-my_one_way(survey_data$importanza.musica)
res_importanza.musica
```
...
```{r}
res_come.ascolti<-my_one_way(survey_data$come.ascolti.musica)
res_come.ascolti
```
...
```{r}
res_concerti<-my_one_way(survey_data$concerti)
res_concerti
```
...
rejecting H0: none
```{r}
res_abitanti<-my_one_way(survey_data$abitanti.citta)
res_abitanti
```
