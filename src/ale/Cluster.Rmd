...
numeric_data$duration<- min_max_normalize(numeric_data$duration)
numeric_data$year<- min_max_normalize(numeric_data$year)
numeric_red<-numeric_data[,-13]
...
```{r}
numeric_red$cluster<-as.factor(clusters)
```


## Feature selection to improve clusters
```{r}
...
# Create a control object for rfe with random forest as the underlying model
ctrl <- rfeControl(functions=rfFuncs, method="cv", number=10)

# Perform Recursive Feature Elimination (RFE)
result <- rfe(numeric_red[, -13], 
              numeric_red[, 13], 
              sizes=c(1:12), 
              rfeControl=ctrl)
...
best_features <- result$optVariables
...
subset<-numeric_red[,best_features]
...
# distance matrix:
data.dist_e <- dist(subset, method=distance)
#data.dist<-mahalanobis.dist(numeric_red)
#temp<-dist(data.dist)
# plot:
#image(1:n,1:n, as.matrix(data.dist), main=paste('metrics: ', distance), asp=1, xlab='', ylab='')
...
fit_RF<-randomForest(as.factor(cluster) ~ ., data = survey_data, mtry=9, ntree=5000, replace=T)
