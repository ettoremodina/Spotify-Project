...
already_done <- function(name_id){
  name_id = paste0(name_id,".csv")
  # Imposta il percorso della cartella
  
  cartella <- "../../data/tracks_info_2/"
...
for (l in link_playlists) {
  ...
  id <- sub(".*/(.+)$", "\\1", l)
  name_id = substr(id, start = 1, stop = 15)
  ...
  if(!already_done(name_id)){
      ...
      if(counter%%3==0){  # every 3 playlists(300 songs) stops for 50 seconds
      Sys.sleep(50)
      }
