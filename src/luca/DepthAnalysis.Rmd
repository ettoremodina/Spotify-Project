...
```{r}
for( i in 1:dim(data_numeric_cleaned)[2])
  print(range(data_numeric_cleaned[,i]))

colnames(data_numeric_cleaned)
```
...
music_vars = music_vars[!(music_vars %in% c("year","duration","id"))]
...
```{r}
val = IWT_covcov("danceability","strumento")
...
results_IWT_list_filter <- list()
alpha_vect = c(0.1)
index = 1
for(a in alpha_vect){
  print(a)
  weights_ecdf <- create_exponential_weights(n, a)
  results_IWT_list_filter[[index]] = covariate_iterator()
  index = index+1
}
```
# plots of ecdf for the ones that passed
```{r}
...
```{r}
grid <- seq(0, 1, length.out = 50)
create_edf_list_2 <- function(data,music_cov){
  edf_list <-  lapply(unique(data$id), function(person_id) {
    ecdf <-ecdf(data[data$id == person_id,music_cov ])
    val = ecdf(grid)
    return(val)
  })
  return(edf_list)
...
```{r}
IWT_covcov_2  <- function(music_cov){ 
 
  soglia_pvalue = 0.05
  
  # Creare una lista di EDF per tutte le persone
  edf_list <- list(create_edf_list_2(data1,music_cov),create_edf_list_2(data2,music_cov))
  matrice_da_lista_all <- do.call(cbind, edf_list)
  response = t(matrice_da_lista_all)
  plot_ecdf_2(music_cov,edf_list)
  
  data1 = t(do.call(cbind, edf_list[[1]]))
  data2 = t(do.call(cbind, edf_list[[2]]))
...
```{r}
plot_ecdf_2 <- function(music_cov,edf_list){
  plot(NULL, xlim = c(0, 1), ylim = c(0, 1),
  main = paste0(c("Empirical Distribution Functions (EDF) ",music_cov)),
  xlab = "Data Values", ylab = "Probability")

  
  for(i in 1:length(edf_list[[1]])){
      lines(grid, edf_list[[1]][[i]], lwd = 1, col = "red", cex = 0.1)
  }
  for(i in 1:length(edf_list[[2]])){
        lines(grid, edf_list[[2]][[i]], lwd = 1, col = "blue", cex = 0.1)
  }
  ...
  legend("topleft", legend = c("group1","group2"), col = c("red","blue"), lwd = 2)
}
