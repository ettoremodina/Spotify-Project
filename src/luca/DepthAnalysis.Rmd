...
edf_list <- lapply(unique(combined_data$id), function(person_id) {
  person_data <- combined_data[combined_data$id == person_id, ]
  ecdf = ecdf(person_data[[covariate]])
  val = ecdf(grid)
  return(val)
})

# Definisci una palette di colori in base ai valori unici nella colonna di gruppo
num_groups <- length(unique(combined_data$Livello.di.educazione))
color_palette <- rainbow(num_groups)

unique_groups = unique(combined_data$Livello.di.educazione)
indices_list <- list()


for (group in unique_groups) {
  indices <- which(questionario$Livello.di.educazione == group)
...
for (i in seq_along(edf_list)) {
  group_value <- questionario$Livello.di.educazione[i + 1]
