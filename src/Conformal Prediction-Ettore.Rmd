...
tabler <- function(s,m){  
  text = c()
  lev = levels(as.factor(combined_data[,s]))
  ...
  for(g in lev ) {
    ...
    temp_level= c(temp_level,paste(c("&","LB",round(list_intervals[[index]][1],3),"UB",round(list_intervals[[index]][2],3)),
                                   collapse = " "))
  }
  
  temp_level = paste(temp_level,collapse = " ")
  temp_text_3 = paste(c(m, temp_level ),collapse= " ")
  combined = c(temp_text,temp_text_2,temp_text_3,"\\end{tabular}","\\end{table}")
  return(combined)
}
...
text = c(text, tabler("estero","acousticness") )
...
plot_CI_2 <- function(temp_list_CI, title, groups_survey) {
  ...
  ggplot(data, aes(x = index, y = value)) +
    ...
    theme(
      legend.position = "top",
      plot.title = element_text(size = 20, hjust = 0.5, vjust = 1) # Centra e fa andare a capo il titolo se è troppo lungo
    )
}
